    ...
    if ($_FILES["filecsv"]["error"] == UPLOAD_ERR_OK) {
        ...
        try {
            foreach ($data as $row) {
                ...
                $query = "INSERT INTO verifikasi_polda (Polda, Satker, Sudah_diupload, Sudah_diverifikasi, Belum_diverifikasi, Ditolak, Ditolak_akumulasi, Persentase, Periode, Triwulan, program, giat, min, max, min_file, max_file)
                          VALUES ('$Polda', '$Satker', '$Sudah_Upload', '$Sudah_Diverifikasi', '$Belum_Diverifikasi', '$Ditolak', '$Ditolak_akumulasi', '$Persentase', '$Periode', '$Triwulan', '$program', '$giat', '$min', '$max', '$min_file', '$max_file')";
                mysqli_query($koneksi, $query);
            }

            $_SESSION['pesan'] = "Data berhasil diunggah dan disimpan.";
            header("Location: ../../../verifikasi/polda/verifikasi.php");
        } catch (Exception $e) {
