...
if($result && mysqli_num_rows($result) > 0) {
    ...
    $Min = 0;
    $Max = 0;
    $sqlMinMax = "SELECT * FROM laporan_polres WHERE periode = '".$dataPersentase['Periode']."' AND PG = '".$dataPersentase['PG']."'";
    ...
    <div class="modal-content">
        ...
        <div class="modal-body ">
                ...
                <div class="form-group mb-2">
                    <label for="Polres" style="font-weight:600;">Satuan Kerja</label>
                    <select name="Polres" class="form-control" id="Polres">';
                        // Query untuk mendapatkan daftar Program Giat
                        $query_jenis = mysqli_query($koneksi, "SELECT DISTINCT Polres FROM persentase_polres");
                        ...
                        // // Menyimpan semua Polres yang sudah ada
                        // $existing_polres = [];
                        // while ($existing_data = mysqli_fetch_array($query_polres)) {
                        //     $existing_polres[] = $existing_data['Polres'];
                        // }

                        // // Reset pointer query untuk digunakan kembali
                        // mysqli_data_seek($query_polres, 0);

                        while ($jenis = mysqli_fetch_array($query_jenis)) {
                            $selected = ($dataPersentase["Polres"] == $jenis['Polres']) ? "selected" : "";
                            $response .= '<option value="' . $jenis['Polres'] . '" ' . $selected . '>' . ucwords($jenis['Polres']) . '</option>';
                        }
