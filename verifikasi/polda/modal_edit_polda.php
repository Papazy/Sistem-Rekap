    ...
    <div class="modal-content">
        ...
        <div class="modal-body ">
                ...
                <div class="form-group mb-2">
                    <label for="Polda" style="font-weight:600;">Satuan Kerja</label>
                    <select name="Polda" class="form-control" id="Polda">';
                        // Query untuk mendapatkan daftar Program Giat
                        $query_jenis = mysqli_query($koneksi, "SELECT DISTINCT Polda FROM persentase_polda");
                        ...
                        // // Menyimpan semua Polda yang sudah ada
                        // $existing_polda = [];
                        // while ($existing_data = mysqli_fetch_array($query_polda)) {
                        //     $existing_polda[] = $existing_data['Polda'];
                        // }

                        // // Reset pointer query untuk digunakan kembali
                        // mysqli_data_seek($query_polda, 0);

                        while ($jenis = mysqli_fetch_array($query_jenis)) {
                            $selected = ($dataPersentase["Polda"] == $jenis['Polda']) ? "selected" : "";
                            $response .= '<option value="' . $jenis['Polda'] . '" ' . $selected . '>' . ucwords($jenis['Polda']) . '</option>';
                        }
